...
export const updatePermissionsValidation: ValidationChain[] = [
  ...
  body('permissions')
    ...
    .bail()
    .custom((value) => value.every((perm: unknown) => typeof perm === 'string'))
    .withMessage('Each permission must be a string')
    .bail()
    .custom((value) => value.every((perm: string) => perm.length >= 2 && perm.length <= 50))
    .withMessage('Each permission must be between 2 and 50 characters')
];
